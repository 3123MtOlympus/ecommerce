...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if(!locationData){
        res.status(404).json({message:'Location not in database'});
        return;
    }
    res.status(200).json(productData);
  } catch (err) {
...
.then((productTagIds) => {
  // If product tags were created, respond with the created product and associated tags
  if (productTagIds) {
    res.status(200).json(productTagIds);
  } else {
    // If no product tags were created, respond with the created product only
